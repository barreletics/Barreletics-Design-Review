...
> **Shop All SIGNED 2026-09-05:** `templates/collection.json` on M4 `187144929571`. Spine: hero → strip → Upgrade + tags → Studio 50/50 → grid 2 rows → **One pair. Done.** → Real people → **Never slip in Chair Pose** fullbleed → **No socks. Just grip.** (discipline tags OFF) → Never loses → mosaic → Knock → Juicer → FAQ. Phone: sock-math 2-up, no “Yoga socks are useless.” Collection fullbleed/50/50 phone frame OFF. No socks pad TE (phone 52). Hub Collection v18 mock stays — never overwrite. Do not restack without a letter.

> **Shop All SIGNED 2026-09-02 night (Andrew: lock it in · good for tonight):** `templates/collection.json` on M4 `187144929571`. Spine: hero → **Upgrade + tags** (Statement **40** · two-line · Barre · Reformer · Megaformer · Lagree · Pilates · Yoga · no deep-lunges sub) → Studio workouts 50/50 (`#faf8f6`) → Shop All **2 rows** + See more → **No socks. Just grip.** → **Never slip in Chair Pose on the fullbleed** (sub + white outline Shop Now → `#grid`) → Never loses → **in-use mosaic (3 tiles)** → Knock → reviews (text cards only · no full Judge.me) → Juicer `@barreletics` → FAQ cream **light `#faf8f6`**. Text-only Never slip **off**. Sole cards off. One cream = light `#faf8f6` (FAQ stays cream, not white). Footer Trusted by light = `#faf8f6` (chrome.css). TE hero crop/width/height + mosaic photos stay user-owned. Do not strip mosaic or Juicer without a letter. Hub Collection v18 mock stays; do not overwrite it.

> **Shop All SIGNED 2026-09-01 (prior):** hero → Upgrade + tags → Studio → grid → mosaic after grid → text Never Slip → fullbleed Commit → Never Loses → Knock → Juicer → reviews → FAQ. Superseded by 2026-09-02 night.
...
> **Home LOCKED 2026-09-09 (Andrew: lock in the photo logic · lock in the home page).** Living spine `templates/index.json` on M4 `187144929571`. Mock lineage `docs/Barreletics Home - Definitive-WORKING.html` — **never overwrite that HTML.** Photo law: **frame stays · photo fills with cover · X/Y/Zoom only.** Skills: `barreletics-images` · `barreletics-home-split-hero`. Rules: `.cursor/rules/barreletics-images.mdc` · `.cursor/rules/home-split-hero.mdc`. Spec: `specs/frozen/homepage.md`. Do not invent an alternate homepage.

> **Home split-hero LOCKED 2026-09-09.** Desktop frame **92vh** · split **62/38** · trust **left** · cover fill · crop **50 / 22** desktop + phone · zoom **100**. **Never** photo-aspect / Section height 110 / undo 92vh / contain + pink pad / crop 0/59. Supersedes the earlier 2026-09-09 “never 92vh” freeze line.
...
> **INTERNI on locked Home 2026-09-09.** `press-feature` after Coperni / before Knock. CTA → `/blogs/news/barreletics-in-interni`. Shop All placement stays after Real people / before Chair Pose until a Collection letter. Do not publish live.
...
**Forward 2026-09-08 — Closed phone QC (M4 `187144929571`).** Liquid only. Home unchanged. Do not restore the 504 text box. **50/50 pad LOCKED 104/104** (Andrew: Think Outside top is the model).
...
| Phone thumbs | All **7** in one row (grid, no clip) |
...
**Forward 2026-09-19 — Home 50/50 desk trial = PDP footprint (Andrew).** Never loses + One pair: `min_height` **560**, `vertical_padding` **80**. Phone **unchanged** (520 / 32 text pads). Big Home beats = fullbleed only. Target: global fifty-fifty consistency; lock after walk. M4 `187144929571`.
...
**Forward 2026-09-15 — Home mobile CSS order — Grip → One Pair → Coperni → Interni.** Andrew: JUST CHANGE THE ORDER · mobile-first · NO REVERT. `barreletics-base.css` ≤768 flex `order` on `#main-content` `[id$="__…"]` only. `index.json` order untouched (desk unchanged). FF 520/520 spot-check OK. No Commit overlay restore. Draft `187144929571`.
...
| **Home** | Homepage `/` | **LOCKED 2026-09-09** · WORKING mock = lineage (never overwrite HTML) | M4 `187144929571` `templates/index.json` + WORKING mock | Spine: hero → mosaic → disciplines → grid → Never loses → Coperni → INTERNI → Knock → fullbleed → reviews → proof → One pair → Chair Pose → Juicer → guarantee. Hero **92vh / 62/38 / trust left / crop 50/22**. Coperni pair **80% squares · cover · 16**. Photo law locked. Do not restack without a letter. | `templates/index.json` + home sections · `specs/frozen/homepage.md` |
