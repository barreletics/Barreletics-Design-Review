...
|---------|--------------|---------------|-----------------|--------------|-----------------|
| **Shop All (grippy shoes)** | `/collections/barre-pilates-yoga-shoe-sock-footwear` | `collection.json` | Hub mock still **Collection v18 Locked** → `docs/Barreletics Collection - Definitive-v18.html` (do not overwrite). **M4 SIGNED 2026-09-05:** hero → strip → Upgrade + tags → Studio → grid 2 rows → One pair. Done. → Real people → Chair Pose fullbleed → No socks (tags OFF) → Never loses → mosaic → Knock → Juicer → FAQ. | *(default / none)* | `/collections/barre-pilates-yoga-shoe-sock-footwear` |
...
| Surface | Handle / URL | Repo template | Hub Locked mock | Admin suffix | QA preview path |
|---------|--------------|---------------|-----------------|--------------|-----------------|
| **Closed Sole** | `/products/best-reformer-pilates-legree-workout-shoes` | `product.json` | **SIGNED 2026-08-16** · living spine = current `product.json` (v19 mock lineage). **No extra wow.** Wow = `fullbleed-lifestyle` 80/60. Prior fingerprint: v16 @ `691f03b` — never overwrite v16/v19 HTML. **Studio FAQ** = Theme settings → Studio FAQ (`use_studio_bank`). **Phone QC 2026-09-08:** ATC no price · Shop Pay under ATC · 50/50 photo 550 · text **104/104 LOCKED** · 7 thumbs fit · reviews 4 + More reviews · TRANSFORM cream 32. See freeze. | *(default / none)* | `/products/best-reformer-pilates-legree-workout-shoes` |
...
| **Better Than Grippy Socks** | `/pages/best-barre-pilates-yoga-grippy-socks` | `page.best-grippy-socks.json` | **SEO v36 Locked** mock stays → `docs/Barreletics SEO - Best Grippy Socks - Definitive-v36.html` (never overwrite). **M4 SIGNED 2026-09-05:** living spine on draft `187144929571` — Full-underfoot grip → shop 2 rows → strip → One pair. Done. → We outgrew → Never loses → Upgrade → reviews → Juicer → FAQ. Footer Learn only. Not Shop All. | **`best-grippy-socks`** ✅ | `/pages/best-barre-pilates-yoga-grippy-socks` |
